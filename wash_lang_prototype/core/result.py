...
class ExecutionResult:
    ...
    def add_attributes(self, **kwargs):
        ...
        In case the instance already has a given attribute, it will update the existing attribute
        by either adding new items to it, or replace the present value.
        """
        for attr_key, attr_value in kwargs.items():
            if not hasattr(self, attr_key):
                ...
                if isinstance(attribute, ExecutionResult):
                    new_attributes = {k: attr_value.__dict__[k] for k in attr_value.__dict__ if k != '__parent'}
                    ...
                    for new_item, existing_item in zip(attr_value, attribute):
                        new_attributes = {k: new_item.__dict__[k] for k in new_item.__dict__ if k != '__parent'}
